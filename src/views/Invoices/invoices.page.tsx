 ...
 * @param props InvoicesPageProps
 * @returns InvoicesPageJSX
 */
function InvoicesPage(props: InvoicesPageProps) {
    const { classes } = props;
                ...
                for (let id in snapshot.val()) {
                    if (id == "jobs") {
                        Object.keys(snapshot.val()[id])
                            .map((key: any) => {
                                if (snapshot.val()[id][key]["invoiceData"]) {
                                    if (filterInputValue !== '') {
    ...
    return isLoadingJobsData ? (
        ...
        <div style={{
            ...
            {
                activeJob && <InvoicesComponent job={activeJob} isOpen={isOpen} setIsOpen={setIsOpen} />
            }
            <GridContainer >
                                                    ...
                                                    tableData={
                                                        !filteredByDate ? (
                                                            jobs[key].map((job: JobsDataInterface) => {
                                                                return [
                                                                    <span style={{ "color": "blue", "textDecoration": "initial", "cursor": "pointer" }} onClick={(e: any) => {
                                                                        e.preventDefault();
                                                                        openInvoiceModal(job);
                                                                    }}>{job.job}</span>,
                                                                    <span style={{ "color": "blue", "textDecoration": "initial", "cursor": "pointer" }} onClick={(e: any) => {
                                                                        e.preventDefault();
                                                                        openInvoiceModal(job);
                                                                    ...
                                                                    <span onClick={(e: any) => {
                                                                        e.preventDefault();
                                                                        openInvoiceModal(job);
                                                                    }}>#{job?.invoiceData?.invoiceTitle}</span>
                                                                ]
                                                            })
                                                        ) : (
...
export default withStyles(dashboardStyle)(InvoicesPage)